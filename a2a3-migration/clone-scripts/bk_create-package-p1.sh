...
INPUT_FILE="./input/sponsored_pages.txt"
OUTPUT_FILE="./output/a2-sponsored"
PACKAGE_NAME="a2-sponsored-p1-backup-$TODAYS_DATE"
CHUNKS=950
USERNAME="admin"
  ...
  if [[ ! -f "${OUTPUT_FILE}/${packageName}.zip" ]]; then
	...
	if [ "${DELETE_MODE}" = "true" ]; then
      printf "Trying to delete the package ${packageName} in the ${HOST} \n"
		...
		printf "${packageName} downloaded from the host ${HOST} to ${OUTPUT_FILE}\n"
